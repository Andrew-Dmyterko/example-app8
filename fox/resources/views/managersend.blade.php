...
@section('content')
    ...
    <h6><b>Адрес - {{ $operPoint->city->city_name }}, {{ $operPoint->point_address }} </b></h6>
        <hr>
        <h5><b>Готовые на отправку посылки:</b></h5>
        <hr>
    <form action="{{route('managerSend')}}" method="get">
    @foreach($packages as $id =>$package)
        <div class="text">
            ...
            <table>
                ...
                <tr>
                    <td>
                        <u><b>Отправка посылки:  </b></u>
                    </td>
                    <td>
                        <input type="checkbox" name="packsend[]" value="{{$package->id}}" id="exampleCheck1" style="margin-left: 5px">  Отправить посылку
                    </td>
                </tr>
            </table>
        </div>
    <hr>
    @endforeach
    <button class="btn btn-primary" name="send_offer" value="send_offer" type="submit">Отправить посылки</button>
    </form>
@endsection
@else
