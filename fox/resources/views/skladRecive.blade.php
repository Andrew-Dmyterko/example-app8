...
@section('content')
    ...
    <h6><b>Адрес - {{ $operPoint->city->city_name }}, {{ $operPoint->point_address }} </b></h6>
        <hr>
        <h5><b>Взодящие посылки ожидающие проверку:</b></h5>
        <hr>
    <form action="{{route('skladPackRecive')}}" method="get">
        ...
        <div class="text">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="packrecive[]" value="{{$package->id}}" id="exampleCheck1" style="margin-left: 5px">  Упаковка/проверка выполнена
                    </td>
                </tr>
                <tr>
                    <td>
                    ...
                    <td>
                        <input type="checkbox" name="bad[]" value="{{$package->id}}" id="exampleCheck1" style="margin-left: 5px">  Посылка с дефектом
                    </td>
                </tr>
            </table>
        </div>
    <hr>
    @endforeach
    <button class="btn btn-primary" name="send_offer" value="send_offer" type="submit">Принять/Принять с деффектами</button>
    </form>
@endsection
@else
